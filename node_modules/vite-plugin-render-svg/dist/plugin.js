...
async function renderFile(svg, scale = 1, optimise = false) {
...
async function getFiles(pattern, hash = false) {
...
function getFileName(fileMeta, scale, ext) {
    let suffix = '';
    if (scale > 1) {
        suffix = `@${scale}x`;
    }
...
export function renderSVG({ pattern, urlPrefix, scales = [1, 2], outputOriginal: copyOriginal = false }) {
    let config;
    let watcher;
    let pattern_path;
    let files;
