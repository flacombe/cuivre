...
Créer l'extension postgis sur la base de données concernée
...
Le référentiel hexaclé utilisé pour identifier les adresses est incomplet et obsolète.
...
Les points géographiques sont créés grâce à la requête :
...
Import :
```bash
psql -c "COPY cuivre_ftthipe (fibre_point, fibre_imb, fibre_num, fibre_voie_type, fibre_voie, fibre_bat, fibre_insee, fibre_commune, fibre_imb_cat, fibre_imb_etat, fibre_pm, fibre_pm_etat, fibre_l33, fibre_imb_type, fibre_date_completude, fibre_date_completude_manquante) from stdin WITH CSV HEADER;" < /tmp/cuivre_ftthipe.csv
```

Export :
```bash
psql -c "COPY(select ST_AsText(fibre_point) as fibre_point, fibre_imb, fibre_num, fibre_voie_type, fibre_voie, fibre_bat, fibre_insee, fibre_commune, fibre_imb_cat, fibre_imb_etat, fibre_pm, fibre_pm_etat, fibre_l33, fibre_imb_type, fibre_date_completude, fibre_date_completude_manquante from cuivre_ftthipe) TO STDOUT WITH CSV HEADER;" > /tmp/cuivre_ftthipe.csv
```
